        ...
        if (e instanceof EventMotion) {
            ...
            if (e.isPre()) {
                ...
                if (pos != null) {
                    ...
                    Vec3 target = PlayerUtils.isMoving() ? new Vec3(this.pos.getX() + 0.5D+ ThreadLocalRandom.current().nextDouble(-0.03D,0.03D), this.pos.getY() + 0.5D+ThreadLocalRandom.current().nextDouble(-0.07D, 0.07D), this.pos.getZ() + 0.5D+ ThreadLocalRandom.current().nextDouble(-0.03D, 0.03D)):new Vec3(this.pos.getX() + 0.5D, this.pos.getY() + 0.5D, this.pos.getZ() + 0.5D);
                    this.rotations = RotationUtils.getNeededFacing(target, from);
                    MovingObjectPosition raytrace = RayTraceUtils.rayTrace(mc.thePlayer.getDistance(this.pos.getX(), this.pos.getY(), this.pos.getZ()) , this.rotations[0], this.rotations[1]);
                    ...
                    if (!mc.gameSettings.keyBindJump.isKeyDown()&& ModuleManager.getModulebyClass(Flight.class).isEnable())
